...
class ConfigParser:
    def __init__(self, args, options='', timestamp=True, slave_mode=False):
        ...
        ##
        category_name = None
        if hasattr(args, 'sub_dir'):
            category_name = args.sub_dir
        if category_name is not None:
            self._save_dir = save_dir / 'models' / exper_name / category_name / timestamp
            self._log_dir = save_dir / 'log' / exper_name / category_name / timestamp
            self._web_log_dir = save_dir / 'web' / exper_name / category_name / timestamp
            self._result_dir = save_dir / 'result' / exper_name / category_name / timestamp
        ##
        else:
            self._save_dir = save_dir / 'models' / exper_name / timestamp
            self._log_dir = save_dir / 'log' / exper_name / timestamp
            self._web_log_dir = save_dir / 'web' / exper_name / timestamp
            self._result_dir = save_dir / 'result' / exper_name / timestamp
        ...
        if vars(args).get("purge_exp_dir", False):
            for dirpath in (self._save_dir, self._log_dir, self._web_log_dir, self._result_dir):
